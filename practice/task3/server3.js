...
let results = [];
    for(const campground of campgrounds){
	if(length <= campground.lengthLimit)
	    {
		results.push({campground: campground.name, location: campground.town,
			maxLength: campground.lengthLimit});
	    }
...
for(const campground of campgrounds){
if(direction == 'higher') {
if(altitude <= campground.elevation){
results.push({campground: campground.name,
	elevation: campground.elevation,
	town: campground.town});
	}
}
